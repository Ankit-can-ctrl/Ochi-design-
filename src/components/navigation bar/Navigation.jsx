...
function Navigation() {
  ...
  //useScroll hook of framer motion gives the current position of scroll
  const { scrollY } = useScroll(); //motion component which tracks scroll

  //useMotionValueevent hook to get on change values instead of using useEffect as no need of cleanup function here
  useMotionValueEvent(scrollY, "change", (latest) => {
    const previous = scrollY.getPrevious(); //to keep track of prev value as well
    if (latest > previous && latest > 150) {
      //as we scroll down latest value becomes bigger than prev value that means we are scrolling down
    ...
    >
      <div className="bg-black text-white font-Founder px-10 py-8">
        <div className=" w-full flex items-center justify-between">
          ...
          <div className="big-links hidden  lg:flex w-[70%] gap-20 justify-end items-center">
            ...
            <Link className="Contact-link ml-48" to="/Contact">
